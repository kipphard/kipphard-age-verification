...
// Load the shared 'kipphard-age-verification' text domain (the Appearance UI strings). English is the
...
if ( function_exists( 'determine_locale' ) && function_exists( 'load_textdomain' ) ) {
	$kipphard_age_verification_mo = __DIR__ . '/languages/kip-' . determine_locale() . '.mo';
	if ( is_readable( $kipphard_age_verification_mo ) ) {
		load_textdomain( 'kipphard-age-verification', $kipphard_age_verification_mo );
	}
}
